...
import { validarCorreo, validarContrasena, validarTelefono } from '../utilidades/validaciones'
...
function Registro() {
  ...
  function validarFormulario() {
    ...
    return !nuevosErrores.correo && !nuevosErrores.contrasena && !nuevosErrores.telefono
  }
  ...
  function manejarCambioTelefono(evento) {
    const valor = evento.target.value
    setTelefono(valor)
    setErrores((anteriores) => ({ ...anteriores, telefono: validarTelefono(valor) }))
  }
  ...
  async function manejarEnvio(evento) {
    ...
    setCargando(true)
    try {
      await api.post('/auth/register', { correo, contrasena, telefono })
      navigate('/login')
    } catch (error) {
      setErrorGeneral(
    ...
    <div className="flex min-h-screen flex-col md:flex-row">
      <div className="flex flex-1 items-center justify-center bg-gray-50 px-6 py-12">
        <form onSubmit={manejarEnvio} className="w-full max-w-sm rounded-xl bg-white p-8 shadow-md">
          ...
          />
          <CampoTexto
            etiqueta="Agrega un número de teléfono:"
            tipo="tel"
            valor={telefono}
          ...
          >
            {cargando ? <Cargando texto="Registrando..." /> : 'Registrar'}
          </button>
